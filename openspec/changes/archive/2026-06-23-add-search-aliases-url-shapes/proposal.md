...
- **Flag shortcuts**: common flags gain single-letter short forms — `-i` (`--instance`), `-s` (`--space`), `-t` (`--title`), `-b` (`--body`), `-l` (`--limit`), `-p` (`--parent`), `-y` (`--yes`) — alongside the existing `-o` (`--output`). These are pure aliases; they change no behavior, only ergonomics. Lower-frequency or destructive-adjacent flags (`--type`, `--cql`, `--start`, `--alias`, `--timeout`, `--insecure`, `--debug`) keep long-only forms.
